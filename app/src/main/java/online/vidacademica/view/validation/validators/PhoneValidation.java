...
public class PhoneValidation implements IValidator {
    private final ValidatorDefaultTextInput validator;
    private final TextInputLayout textInputLayout;
    private final Context context;

    public PhoneValidation(Context context, TextInputLayout textInputLayout) {
    ...
    public boolean validation(Boolean setError) {
        ...
        if (Util.validatePhoneNumber(text)) {
            if (textInputLayout.getEditText() != null && setError)
                textInputLayout.setError(context.getString(R.string.invalid_phone_number).toUpperCase());
            return false;
        }
        return true;
    }
